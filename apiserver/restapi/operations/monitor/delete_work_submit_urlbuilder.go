...
func (o *DeleteWorkSubmitURL) Build() (*url.URL, error) {
	...
	var _path = "/v1/workSubmit/{id}"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on DeleteWorkSubmitURL")
	}
...
func (o *DeleteWorkSubmitURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DeleteWorkSubmitURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DeleteWorkSubmitURL")
	}
